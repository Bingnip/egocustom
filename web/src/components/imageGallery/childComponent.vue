<template>
    <el-button plain @click="openGalleryDialog">
        Open a overflow draggable Dialog
    </el-button>
    <el-dialog v-model="galleryVisible" close-on-press-escape="true" title="上传商品图"
        style="margin-top: 15vh; width: 1020px;" draggable overflow>
        <div>
            <el-container>
                <el-aside width="200px">
                    <el-tree style="max-width: 550px" @node-click="checkedTreeNode" :data="folderList"
                        :props="defaultProps">
                ...
                <el-container>
                    <el-header>
                        ...
                        <el-select @blur="moveImg" clearable :disabled="imgCheckedArr.length == 0" v-model="folderValue"
                            value-key="item.af_id" placeholder="图片移动至" style="width: 170px; margin-left: 10px;">
                            <el-option v-for="item in usedFolderList" :key="item.af_id" :label="item.af_name"
                                :value="item.af_id" />
                        </el-select>
                    </el-header>
                    <el-main class="img-block-container">
                        ...
                        <div v-else v-for="(it, idx) in imageList" class="img-block" :key="it.id">
                            <div class="img-block-parent" @click="imgCheckHandler(idx)">
                                <img :class="['img-block-inner', {'img-block-inner-checked': it.checked}]"
                                    :src="cdnUrl + it.url" :alt="it.name">
                            ...
                            <div @mouseenter="hoveredIndex = idx" @mouseleave="hoveredIndex = null">
                                <div class="img-title-block" style="height: 26px;">
                                    ...
                                    <el-input @blur="renameSubmit(idx)" size="small" v-else v-model="it.name">
                                    </el-input>
                                </div>
                    ...
                    <el-footer class="footer">
                        <el-pagination v-model:current-page="pagination.currentPage" :page-size="pagination.pageSize"
                            :size="small" :disabled="disabled" background layout="total, prev, pager, next"
    ...
    <el-dialog v-model="uploadDialogVisible" title="上传图片" width="68%">
        <el-form :model="uploadDialogForm" label-width="auto" size="small" style="margin-left: 20px;">
            ...
            <el-form-item label="上传图片：">
                <el-upload v-model:file-list="uploadDialogForm.uploadImgs" list-type="picture-card" multiple
...
<script lang="ts" setup>
    ...
    const imgCheckHandler = (idx) => {  // 图片点击选中操作
    ...
    const scanCheckedPush = (idx) => {  // 选中
        imgCheckedArr.value.push(imageList.value[idx])
        resetCheckedNo()
    }
    const scanCheckedRemove = (id) => {  // 移除
    ...
    const resetCheckedNo = () => {  // 重新计算选中图片的编号
        if (imgCheckedArr.value.length > 0) {
            imgCheckedArr.value.forEach((e, i) => {
    ...
    const deleteImgSingle = (idx) => { // 单独删除

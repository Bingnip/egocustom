<template>
    ...
    <el-dialog class="ba-operate-dialog" :close-on-click-modal="false"
        :model-value="['Add', 'Edit'].includes(baTable.form.operate!)" @close="baTable.toggleForm" width="55%"
        :style="{ height: '800px' }">
        <template #header>
        ...
        <div class="ba-operate-form" :class="'ba-' + baTable.form.operate + '-form'"
            ...
            <el-form v-if="!baTable.form.loading" ref="formRef" @submit.prevent=""
                ...
                :rules="rules">
                <FormItem :label="t('siteManage.currencyConfig.crc_code')" type="string"
                    v-model="baTable.form.items!.crc_code" prop="crc_code" block-help="货币Code，例如：USD"
                    :input-attr="{ maxlength: 3 }"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_code') })"
                    :inputAttr="{ style: { width: '130px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_name')" type="string"
                    v-model="baTable.form.items!.crc_name" prop="crc_name" block-help="例如：US Dollar"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_name') })"
                    :inputAttr="{ style: { width: '200px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_sign')" type="string"
                    v-model="baTable.form.items!.crc_sign" prop="crc_sign" block-help="例如：$"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_sign') })"
                    :inputAttr="{ style: { width: '130px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_icon')" type="image" limit="1"
                    v-model="baTable.form.items!.crc_icon" prop="crc_icon"
                ...
                <FormItem :label="t('siteManage.currencyConfig.crc_rate')" type="number" prop="crc_rate"
                    v-model.number="baTable.form.items!.crc_rate" block-help="（100美元可兑换多少此种货币）"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_name') })"
                    :inputAttr="{ style: { width: '120px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_exchange_rate')" type="number"
                    prop="crc_exchange_rate" v-model.number="baTable.form.items!.crc_exchange_rate"
                    ...
                    :inputAttr="{ style: { width: '120px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_default')" type="switch" prop="crc_default"
                    v-model.number="baTable.form.items!.crc_default" block-help="(是否为默认的货币)"
                    :data="{ content: { '1': '是' } }"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_default') })" />
                <FormItem :label="t('State')" type="radio" v-model="baTable.form.items!.crc_status" prop="crc_status"
                    :data="{ childrenAttr: { border: true }, content: { '0': '禁用', '1': '启用' } }"
                    :placeholder="t('Please select field', { field: t('State') })" />
                <FormItem :label="t('siteManage.currencyConfig.crc_order')" type="number" prop="crc_order"
                    :input-attr="{ step: 1 }" v-model.number="baTable.form.items!.crc_order"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_order') })"
                    :inputAttr="{ style: { width: '80px' } }" />
                <FormItem :label="t('siteManage.currencyConfig.crc_memo')" type="textarea"
                    v-model="baTable.form.items!.crc_memo" prop="crc_memo"
                    :placeholder="t('Please input field', { field: t('siteManage.currencyConfig.crc_memo') })"
                    :input-attr="{ rows: 3 }" :inputAttr="{ style: { width: '700px' } }" />
            </el-form>
        </div>
        <template #footer>
